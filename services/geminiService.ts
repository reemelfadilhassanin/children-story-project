...
export async function describeHero(photo: File): Promise<string> {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/describe-hero`, {
            method: 'POST',
            body: formData,
        });
        ...
        if (!response.ok) {
            try {
                ...
                throw new Error(errorData.error || "Failed to analyze the hero's photo.");
            } catch (jsonError) {
                 // If the response is not JSON, use the status text.
                 throw new Error(response.statusText || "Failed to analyze the hero's photo.");
            }
        }
        ...
        if (error instanceof Error) {
            throw new Error(error.message || "Failed to communicate with the server to analyze the photo.");
        }
        throw new Error("An unknown error occurred while analyzing the photo.");
    }
}

export async function generateStory(theme: StoryTheme, characterDescription: string, childName: string, artStyle: string): Promise<Story> {
    try {
        ...
        if (!response.ok) {
            try {
                 const errorData = await response.json();
                 throw new Error(errorData.error || "The AI failed to create a story.");
            } catch(jsonError) {
                throw new Error(response.statusText || "The AI failed to create a story.");
            }
        }
        ...
        if (!storyData.title || !storyData.pages || !Array.isArray(storyData.pages)) {
             throw new Error("The server returned an invalid story structure.");
        }
        ...
        console.error("Error generating story:", error);
        if (error instanceof Error) {
           throw new Error(error.message || "Failed to communicate with the server to generate the story.");
        }
...
export async function generateImage(prompt: string): Promise<string> {
    try {
        ...
        if (error instanceof Error) {
            throw new Error(error.message || "Failed to communicate with the server to generate the image.");
        }
